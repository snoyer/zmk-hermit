...
def main():
    ...
    parser.add_argument(
        "board", metavar="BOARD", help="ZMK board name or out-of-tree board directory"
    )
    parser.add_argument("--keymap", metavar="FILE", help="out-of-tree keymap file")
    ...
    group.add_argument("--zmk-image", metavar="IMAGE", help="Docker ZMK-build image id")
    ex = group.add_mutually_exclusive_group()
    ex.add_argument("--zmk-git", metavar="URL", help="ZMK git repository url")
    ex.add_argument("--zmk-src", metavar="DIR", help="ZMK source directory")
    group.add_argument(
        "--setup", action="store_true", help="initialize and update build environment"
    )
    group.add_argument(
        "--build-dir", metavar="DIR", help="directory to store build files into"
    )
    group.add_argument("--zmk-config", metavar="IMAGE", help="ZMK-config dir")
    group.add_argument(
        "-p",
        "--pristine",
        action="store_true",
    ...
    group.add_argument(
        "--usb", choices=["y", "n"], nargs="?", const="y", help="set CONFIG_ZMK_USB"
    )
    group.add_argument(
        "--bt", choices=["y", "n"], nargs="?", const="y", help="set CONFIG_ZMK_BLE"
    )
    group.add_argument(
        "--max-bt",
        type=int,
    ...
    group.add_argument("--kb-name", help="set CONFIG_ZMK_KEYBOARD_NAME")
...
def run_build(args: argparse.Namespace, extra_args: Sequence[str]):
    ...
    if args.zmk_config:
        zmk_config_path = Path(args.zmk_config)
    ...
    if not exit_code and args.into:
        for fn in Path(args.into).glob(f"{output_basename}.*"):
            if (
                fn.suffix.lstrip(".") in args.extensions
